...
- **People** (`people`) — general words for persons: man, woman, child, baby,
  ...
  Ex: ji'nm (man), e'pit (woman), mijua'ji'j (baby), witapal (friend).
- **Peoples & Nations** (`peoples-and-nations`) — Mi'gmaq, French, English,
...
- **Learning & Teaching** (`learning-and-teaching`) — learning, teaching,
...
- **Hearing & Sounds** (`hearing`) — hearing, listening, and sounds: loud,
  ...
  Ex: nutuatl (hear), jigs'tg (listen), gesigawweg (loud), jijuejg (bell).
- **Smell, Taste & Touch** (`smell-and-taste`) — odors, smelling, flavours,
  tasting-as-sensing, touching, tickling, rubbing, numbness.
  Ex: gesleg (stink), welima'q (smell good), sama'latl (touch), pisse'g (numb).
...
- **Holding & Grabbing** (`holding`) — hold, grip, grab, catch, keep, save,
  put away, store.
  Ex: geggunatl (have/hold), goqqwa'latl (grab), masqwa'toq (store away).
- **Pushing, Pulling & Throwing** (`pushing-and-pulling`) — push, shove, pull,
  drag, tow, stretch, throw, fling, drop.
  Ex: gesma'latl (push), wetnatl (pull), sewtaqa'latl (tow), elaqtoq (fling).
- **Hitting & Striking** (`hitting`) — hit, beat, knock, pound, kick, spank,
  slap, shooting at (as striking).
  Ex: etlta'tl (beat), taqamatl (punch), tegtesg'g (kick), matte'g (spank).
- **Cutting & Piercing** (`cutting`) — cut, chop, slice, carve, saw, whittle,
  shave, stab, pierce, sharpen.
  Ex: temsatl (cut), eloqsatl (carve), sa'p'g (pierce), esipulatl (sharpen).
- **Breaking & Crushing** (`breaking`) — break, smash, tear, rip, crush,
  shatter, burst.
  Ex: tema'latl (break in two), nugjaqta'tl (smash), pasgina'latl (tear).
- **Tying, Fastening & Wrapping** (`fastening`) — tie, bind, attach, hook,
  lock, wrap, cover, open and close, plug, seal.
  Ex: gelpilatl (tie up), megwasa'latl (stick on), apt'sqa'q (lock up),
  pana'toq (open), gepsaqa'toq (close).
- **Falling & Tipping** (`falling`) — falling, toppling, tipping over,
  collapsing, caving in.
  Ex: nisiet (fall down), gewiet (topple), seioqiet (collapse).
...
- **Cooking & Baking** (`cooking`) — cooking, baking, boiling, roasting,
  ...
  Ex: etloqsatl (cook), epipnet (bake bread), wejaqama'latl (boil), sisla'gwa'latl (fry).
...
- **Houses & Buildings** (`dwellings`) — wigwams, houses (and the big
  house-by-material family), barns, walls, roofs, building structures.
  Ex: wigwom (wigwam), wenji'guom (house), guntewi'gan (stone house), elsaqas'g (wall).
- **Cleaning & Washing** (`cleaning`) — cleaning, washing, sweeping, wiping,
  scrubbing, laundry, soap, bathing-as-washing.
  Ex: waqama'toq (clean), gesispa'latl (wash), pewi'get (sweep), gsispa'taqan (soap).
...
- **Work & Effort** (`work`) — working, jobs, tasks, effort, working
  hard/fast/poorly, skill at work.
  Ex: elugwet (work), lugowaqan (job), attigna'sit (work hard), gisa'toq (able to fix/do).
- **Occupations** (`occupations`) — the person-words for trades and roles:
  teacher, doctor, baker, barber, fisher, worker.
  Ex: gegina'muet (teacher), malpale'wit (doctor), nujipipnet (baker), lugowinu (worker).
- **Tools** (`tools`) — tools and implements: axe, hammer, knife, drill, file,
  shovel-as-object, rope and line, gear in general.
  Ex: tmi'gn (axe), maltejjuei (hammer), wa'qan (knife), po'qign (drill bit).
- **Money, Buying & Selling** (`money-and-trade`) — money, paying, prices,
  buying, selling, peddling, owing, wealth and poverty.
  Ex: suliewei (money), apangitg (pay for), pegwatelg (buy), netuisget (sell),
  tettuet (owe), milesit (rich).
- **Farming & Gardening** (`farming`) — plowing, planting, harvesting, hay,
...
- **Animals** (`animals`) — wild mammals and general animal words; also
  reptiles, frogs, turtles, snakes.
  Ex: tia'm (moose), mui'n (bear), apli'gmuj (rabbit), migjigj (turtle), wi'sis (animal).
- **Farm Animals & Pets** (`farm-animals`) — domestic animals: cows, pigs,
  horses, sheep, goats, chickens, dogs, cats.
  Ex: wenjitia'm (cow), gulgwi's (pig), te'sipow (horse), gajuewj (cat), lmu'j (dog).
...
- **Trees & Forest** (`trees`) — tree species, wood, bark, logs, lumber,
  stumps, the forest.
...
- **Sky & Heavens** (`sky`) — sun, moon, stars, northern lights, rainbow,
  comets, daylight and darkness of the sky.
  Ex: na'gu'set (sun), tepgunset (moon), gloqowej (star), waqatasg (northern lights).
...
- **Boats & Paddling** (`boats`) — canoes, boats, ships, paddling, rowing,
  sailing, anchoring, going by water.
  Ex: gwitn (canoe), ji'met (paddle), alaqteget (sail about), eguma'toq (anchor).
- **Swimming & Floating** (`swimming`) — swimming, wading, diving, floating
  (of people and things).
  Ex: ala'q (swim about), tegismit (swim), asoqomasugwet (wade across), getapet (dive).
- **Fishing** (`fishing`) — fishing as activity and gear: nets, hooks, lines,
  spears, weirs, fishing for particular fish.
  Ex: egwitamet (fish), a'pi (net), mgign (fish hook), nigoq (fish spear), wesget (fish).
...
- **Size & Shape** (`size-and-shape`) — big, small, long, short, wide, narrow,
  round, flat, crooked, pointed, and measuring size.
  Ex: mesgi'g (big), apje'jg (small), pittaq (long), piptoqopsgeg (round),
  istuig (crooked).
- **Colours** (`colors`) — colour words and colour-changing.
  Ex: megwe'g (red), maqtawe'g (black), wape'g (white), ewne'g (blue), stoqonamu'g (dark green).
- **Texture & Materials** (`texture-and-materials`) — soft, hard, sticky,
  slimy, smooth, rough; materials like metal, glass, stone-as-material.
  Ex: nugwe'g (soft), melge'g (hard), peguig (sticky), qasawo'q (iron), pig'lew (glass).
- **Hot & Cold** (`temperature`) — warm, hot, cold, cool, freezing, thawing,
  melting; feeling warm or cold.
  Ex: epteg (hot), tegig (cold), geljit (frozen), apua'latl (thaw), gewjit (feel cold).
- **Numbers & Counting** (`numbers`) — number words, counting, ordinals, the
  counting-classifier families, arithmetic.
  Ex: newt (one), ta'pu (two), na'n (five), gasg'ptnnaqan (hundred), mawgiljet (add up).
- **Amounts & Degree** (`amounts`) — many, few, much, little, full, empty,
  enough, more, too much, all, none, leftovers, halves and portions.
  Ex: pugwelg (lots), tepiaq (enough), wesamelg (too much), esgwiaq (leftover),
  a'qataig (half).
- **Good & Bad** (`good-and-bad`) — good, bad, better, worse, best, worst,
  right and wrong, proper, useful and useless, precious.
  Ex: gelu'lg (good), winjig (bad condition), ajiglu'lg (better), tetapu'teg (correct).
- **Condition & State** (`condition`) — the physical state of things: new,
  old (of things), worn out, ripe, rotten, sharp, dull, stuck, loose, tight,
  tired, dry, brittle.
  Ex: pilei (new), sa'qawei (old), gaqiet (worn out), essit (ripe), sugulegaq
  (rotten), amjimoqteg (stuck), gispneg (tired).

## Time

- **Time** (`time`) — time words: now, later, soon, today, yesterday, morning,
  night, always, sometimes, while; o'clock and durations.
  Ex: nige' (now), gisgug (today), sapo'nug (tomorrow), egsitpu'g (morning),
  apjiw (always).
- **Calendar & Special Days** (`calendar`) — months, weekdays, weeks, years,
  seasonal calendar, holidays and feast days: Christmas, Easter, Sunday.
  Ex: apignajit (February), gesp'teg (Saturday), Nuelewumg (Christmas Day),
  aqantie'uti (week), siggw (spring).
- **Age & Growing** (`age`) — young, old (of people and animals), growing up,
...
- **Place Names** (`places`) — proper names of places: communities, districts,
  towns, rivers and landmarks with names.
  Ex: Listuguj, Gespe'g, Esg'soqonig (Eskasoni), Ganata (Canada), Mulian (Montreal).
- **Here, There & Where** (`location-words`) — position-in-space words: here,
  there, near, far, inside, outside, under, on top, middle, edge, beside.
  Ex: tet (here), na'te'l (there), gigjiw (near), gneg (far), lame'g (underneath),
  gujm (outside).
- **Directions** (`directions`) — compass points and oriented directions:
  north, south, east, west, left, right, upstream, downstream.
  Ex: oqwatn (north), gp'te'sn (south), ugjipen (east), tg'sn (west), inaqan (right side).
- **Question Words** (`question-words`) — what, who, where, when, which, why,
  how much.
  Ex: goqwei (what), wen (who), tami (where), ta's'g (how much), tegen (which).
- **Small Connecting Words** (`small-words`) — grammatical particles: and,
  but, or, also, perhaps, not, not-yet, because, still, very.
  Ex: aq (and), gatu (but), gisna (or), etug (perhaps), moqwa' (no), muta (because).
- **Greetings & Expressions** (`greetings`) — greetings, farewells, thanks,
  politeness, exclamations and interjections.
  Ex: pusu'l (greetings!), atiu (goodbye), wela'lin (thank you), pjila'si
  (welcome), etnana (that's it!).
...
- **top-10** — the absolute first words: hello, thank you, yes, no, goodbye,
  mother, water, eat... (exactly 10)
- **top-100** — first-conversations vocabulary: core kinship, numbers 1-10,
  question words, common foods and animals, key verbs (eat, sleep, go, see,
  want, know), greetings. (exactly 100, including the 10)
- **top-1000** — a working everyday vocabulary across all topics, preferring
  words with recordings. (exactly 1000, including the 100)

**Caveat for reviewers:** these tiers are an *aspirational learning order*
proposed from English-side intuition, not Mi'gmaq corpus frequency. The
language team should audit top-10 and top-100 word-by-word; disagreements
there matter much more than anywhere else in this project.
