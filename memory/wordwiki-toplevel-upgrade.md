...
- **Auth:** all /ww/ routes gated behind login (rewriteUnauthenticatedRoute); GET login allowed on non-production (`/ww/wordwiki.loginRequest(queryArgs)?username=djz&password=...`); sessions are db rows (revocable, survive restarts). The two legacy root endpoints (/page/..., /workspace-rpc-and-sync) bypass the gate until the old editor retires. **Dev db marked db_purpose=dev; dev password: djz / djz-dev** (set via `./wordwiki.sh set-password djz djz-dev`). `change_by_username` is now stamped from the session in every lexeme-editor assertion (changeStamp()).
...
- **Production pulls (2026-06-11):** the dev db is now a fresh production pull. `./wordwiki.sh post-pull [pw]` makes a pulled prod db dev-runnable (replaces old-shape empty user table, seeds 17 users, marks dev, sets djz password, default djz-dev) — idempotent, and `pullDbFromPublic.sh` (now versioned) runs it automatically after the rsync. Needed until the new version IS production. dz is starting a batch re-categorization project for lexemes (stats from prod: 8,822 entries, 92% with glosses, 264 messy hand categories incl. `_`×232 and a long once-used tail).
