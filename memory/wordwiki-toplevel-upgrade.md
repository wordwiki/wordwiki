...
- **Production pulls (2026-06-11):** the dev db is a fresh production pull. `./wordwiki.sh post-pull` makes a pulled prod db dev-runnable (replaces old-shape empty user table, seeds users + passwords from user-passwords.json, marks dev) — idempotent; Phase 2 of the rebuild pipeline calls it after copying the snapshot (see [[rebuild-pipeline]]; the old pullWordWikiV1Db.sh was retired 2026-08-02). Needed until the new version IS production. dz is starting a batch re-categorization project for lexemes (stats from prod: 8,822 entries, 92% with glosses, 264 messy hand categories incl. `_`×232 and a long once-used tail).
