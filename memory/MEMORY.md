...
- [Wordwiki decomposition](wordwiki-decomposition.md) — WordWiki class split; ALL 7 phases DONE (cli.ts, DictionaryStore, SiteView-per-orthography, workingSite(), wordwiki.reports.*, site-config.ts + books-from-db + book-param reports); residuals listed inside; top-level `await import(cli.ts)` deadlocks — use .then()
- [Wordwiki archival publish model](wordwiki-archival-publish-model.md) — dz's 4-stage archival philosophy; publish-via-JSON-intermediate proposal (endorsed, NOT built); customization = db config + factory-getter subclassing (not mytitan nested classes)
