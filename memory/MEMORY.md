...
- [Wordwiki decomposition](wordwiki-decomposition.md) — WordWiki class split plan; phases 1-5 DONE (cli.ts, DictionaryStore, SiteView-per-orthography, workingSite() render selection; picker/publish stay pinned to the public view); next: report-route namespaces, config pass; top-level `await import(cli.ts)` deadlocks — use .then()
