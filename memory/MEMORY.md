...
- [Wordwiki decomposition](wordwiki-decomposition.md) — WordWiki class split plan; phases 1-6 DONE (cli.ts, DictionaryStore, SiteView-per-orthography, workingSite(), wordwiki.reports.* namespace w/ narrow ReportsApp iface); wordwiki.ts 2488→915 lines; remaining: config pass on Mi'kmaq constants; top-level `await import(cli.ts)` deadlocks — use .then()
