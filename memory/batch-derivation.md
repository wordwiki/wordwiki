...
PDM retrofit DONE 2026-08-06 (53572b6): `pdm-import --batch` = derivePdm derive cycles (pure, ZERO db writes, per-page units, exit 3 in flight, rerun to continue) → when every page is 'complete' the UNCHANGED importPdm lands the whole mirror from pure cache (commit-at-end for the entire import; the wipe only happens with all AI in hand). cropClosure = THE shared crop key computation (transcribe.ts, §12b); boxesCropPath derives crops from raw segmentation rects (no bounding group); readEntryFromCrop = the DUAL-MODE op (dz's requirement: bulk batch / interactive re-derivation share keys + cache). llmRetry + the pdm fallback catches pass DerivationNotAvailable through (a deferred grouping must NOT trigger the paid starts-fallback). BOTH ban gates passed: sync relands gen-5 byte-identically (747/7309, zero AI); --batch derives all 304 pilot readings via the new no-group route with ZERO enrollments against the real paid cache.

Remaining:
- The big deferred jobs ([[deferred-llm-runs]] — full rand binder via bind-references --batch, full-book PDM via pdm-import --batch) now ride at half price — still need dz's explicit go + quote.
