...
people CARE about (member names inline, not "5 members"); items render as soft
tappable blocks (`.lm-doc-section` — faint fill + rounding, no border; the block
IS the touch affordance) or a compact `tr.lm-navigable` table; NO per-row
pencils — edit whole-record params on the DETAIL page; inline affordances only
where a per-item detail round-trip would be excessive (a task's checklist:
toggle inline, Edit in the item's ☰).  View state rides the URL (page-state,
