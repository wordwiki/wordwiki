...
**Data-access cousin (bitten 2026-07-07, reference-page search):** routeterp gates EVERY member access, including on plain data — a route *expression* like `renderTextSearchResults(4, {...}, query.searchText)` 404s because `searchText` isn't a @route on the query-args object. Route expressions must pass bindings WHOLE (`..., query)`, the `searchPage(query)` convention) and destructure server-side. Grep emitted URLs for `query.` when hunting these.
