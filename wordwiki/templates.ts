...
// Bootstrap Icons "clock-history" (MIT), inlined like eyeIcon/pencilIcon.
export function clockHistoryIcon(): any {
    return ['svg', {viewBox: '0 0 16 16', fill: 'currentColor', 'aria-hidden': 'true'},
...
/** A full-history link (dz: a clock beside the editor title's eye - the
 *  word's complete change record must be reachable even when nothing is
 *  pending).  Same quiet icon-affordance treatment as the eye/pencil. */
export function historyLink(historyUrl: string,
                            opts: {extraClass?: string} = {}): any {
    const cls = 'lm-history-clock' + (opts.extraClass ? ` ${opts.extraClass}` : '');
    return ['a', {...pageLinkProps(historyUrl), class: cls,
                  'aria-label': 'History', title: 'Full history'}, clockHistoryIcon()];
}
          ...
          ['div', {class:'collapse navbar-collapse', id:'navbarSupportedContent'},
           ['ul', {class:'navbar-nav me-auto mb-2 mb-lg-0'},
            ...
            `Entries by ${siteConfig.primarySourceBook} page`]],
              ...
              // (The standalone review PAGES moved to the Reviews menu.)
            ...
            // --- Reviews.  A STABLE, findable home for the standalone
            //     human-review pages (dz 2026-07-31: experts keep losing
            //     emailed URLs; an in-app menu link is the fix).  These are
            //     generated resources/*.html artifacts (re-generated by
            //     their tools) - plain hrefs, not app fragments.
            ['li', {class:'nav-item dropdown'},
             ['a', {class:'nav-link dropdown-toggle', href:'#', role:'button', 'data-bs-toggle':'dropdown', 'aria-expanded':'false'},
              'Reviews'],
             ['ul', {class:'dropdown-menu'},
