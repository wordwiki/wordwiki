...
/**
 * Recently changed WORDS: a backwards feed of lexemes, ordered by the most
 * recent human change to ANY assertion composing the word, clumped by week.
 *
 * This is the reviewer's front door for word-at-a-time approval
 * (meta-editor-changes-mode.md): walk the list top to bottom, tap a word to
 * open its VIEW-CHANGES page (baseline→current annotations in context),
 * Approve all, come back.  The per-assertion global feed (change-feed.ts)
 * stays for the tricky situations; a reviewer working this list never has to
 * touch the complex representation.
 *
...
import {FieldSet, IntegerField, TimestampField, type Tuple} from '../liminal/table.ts';
...
export class RecentWords {
    ...
    page(q?: Record<string, any>): templates.Page | server.Response {
        ...
        const body = ['div', {class: 'container py-3'},
            ['div', {class: 'd-flex align-items-center gap-2 mb-1 flex-wrap'},
             ['h2', {class: 'mb-0 me-2'}, 'Recently changed words']],
            ['div', {class: 'text-muted small mb-3'},
             'Each word once, ordered by its newest change. Tap a word to see '
             + 'what changed, in context.'],
            rows.length === 0
                ? ['div', {class: 'text-muted'}, 'No changed words.']
                : this.weekGroups(rows).map(g => this.renderWeek(g))];
        return templates.page('Recently changed words', body);
    }
    ...
    /** One row per WORD: its newest human change (same exclusions as the
     *  change feed: no imported base set, no '~' automated identities, no
     *  born-published corpus - CHANGE_ROW), newest first.  SQLite's
    ...
    private fetchWords(query: RecentWordsQuery): WordRow[] {
        ...
        // whole-table SQL.
        return rows.map(r => ({entry_id: r.entry_id, last_change: r.last_change,
                               username: r.username ?? '',
