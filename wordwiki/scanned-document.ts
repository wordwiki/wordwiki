...
/**
 * The scanned-document world: the (legacy raw-DML) SQL tables behind the
 * transcription workflow - scanned documents and their pages, tagging layers,
 * bounding groups/boxes (+ full-text search), and the change log.
 *
 * The other data world - the dict assertion table - lives in assertion.ts;
 * createAllTables (both worlds' DML) is on wordwiki.ts.
 */
import { db, assertDmlContainsAllFields, boolnum } from "../liminal/db.ts";
...
// All of this module's table DML, for db creation (see createAllTables in
// wordwiki.ts).
export const allScannedDocumentSchemaDml =
