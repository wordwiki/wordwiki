...
// --------------------------------------------------------------------------------
//
// Built through the framework like any other table, so it auto-creates,
// schema-evolves, and gets the default editor UI.  Its first use is `db_purpose`,
// a marker that travels with the data and lets destructive operations (test
// setup, fake-data rebuilds) refuse to clobber a production database.
//
// Protection is opt-in by marking: a real database must be marked 'production' to
// be protected; an unmarked one is treated as wipeable (with a warning), so
// dev/legacy databases keep working.
...
export const configMetaData = new ConfigTable();
