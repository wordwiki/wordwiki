* TODAY Get login working
* TODAY Get generic editing working
* TODAY Get generic actions working
* TODAY fix activity report.



* TODO make activeVolunteersByDay report
- update fake data so is nice
- make same pages show free bikes, trainings etc by month.
- 


* Primary paper use
- have a sheet per event, with all the spots on it (double sided)
- at end of event, enter into rabid.
- screen for turbo entry, list of postal prefixes for service (XXX for missing) all on one string. (or some picker).
- list of volunteers (common ones at top), click to add.
- bike sales etc entered in a standard format.

* npm installed
deno install npm:@faker-js/faker
deno install npm:temporal-polyfill

* PROPOSAL: Timer function in rabid
- once someone is put on a DIY stand, start timer with the stand #
- have page that shows time on stand for all DIY clients.
- TUE/WED/FRI - 3 days to working rabid?  Is it worth it?  Maybe?

 
* TODO Login
- on every request, if don't have session token that resolves to active session:
- display login page instead (keeping URL).
- on submit, with valid login, just refresh same URL, and will display.
* TODO Account creation
- for now, new volunteers are assigned a random password - which is emailed to them.
- because passwords are not stored in the DB, creator will need to copy etc.
- they can change the password.

* TODO Write nice date library
- convert to/from sqlite format into best JS format.
- hopefully from there will get date aritmetic.
