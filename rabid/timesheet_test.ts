...
import { lateEntryWarning, lateEntryBadge, lateEntryAlert } from "./timesheet.ts";
...
test("paid + entered >24h after end -> 'entered' warning", () => {
    const d = lateEntryWarning(entry({entry_creation_time: '2026-06-04 12:00:00',   // 3 days
...
test("paid + entered on time but edited >24h after end -> 'edited' warning", () => {
    const d = lateEntryWarning(entry({entry_creation_time: '2026-06-01 13:00:00',   // prompt
...
test("paid + recorded within 24h -> no warning", () => {
    assertEquals(lateEntryWarning(entry({entry_creation_time: '2026-06-02 09:00:00',
                                               entry_last_edit_time: '2026-06-02 09:00:00'})), null);
});

test("UNPAID entered very late -> no warning (only paid time matters)", () => {
    assertEquals(lateEntryWarning(entry({is_paid_time: 0,
...
test("open paid entry (no end_time) -> no warning yet", () => {
    assertEquals(lateEntryWarning(entry({end_time: null,
                                               entry_creation_time: '2026-07-01 12:00:00'})), null);
});

test("renderers: badge/alert present for a finding, absent (undefined) otherwise", () => {
    const d = lateEntryWarning(entry({entry_creation_time: '2026-06-05 12:00:00',
                                            entry_last_edit_time: '2026-06-05 12:00:00'}))!;
    const badge = lateEntryBadge(d);
    assert(findByTestId(badge, 'late-entry'), 'badge renders');
    assert(hasText(badge, 'late entry'));
    assert(hasText(lateEntryAlert(d), 'hazy reconstructions'));
    assertEquals(lateEntryBadge(null), undefined);
    assertEquals(lateEntryAlert(null), undefined);
});
