...
**1,078 distinct RAND Listuguj spellings (1,279 field occurrences) use a backtick `` ` ``** (none in the
...
After the mechanical rules (finding 2 + case) are applied, **505 near
...
|--------------------------|-----------------------:|--------------------------:|
| MMO writes, RAND omits   | **336** (`esa'tl` vs `esatl`) | 19 |
| RAND writes, MMO omits   | 95 (`enga'latl` vs `engalatl`) | **73** (`engat'g`, `nmu'j`-style) |

Reading: **MMO marks vowel length much more consistently than the
RAND transcription** (336 vs 95), while **Watson marks schwa (with
`` ` `` or `'`) more than MMO does** (the backticks + 73 vs 19).
...
- The mechanical ceiling is low: +85 exact matches (~15% of the near
  pairs).  The remaining 505 are the length/schwa marking practices of finding 3 —
...
generation 1 of both imports; all figures regenerated by the
script (distinct-spelling counts).
